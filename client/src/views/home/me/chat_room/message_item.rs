...
pub fn message_item(Props { message }: &Props) -> Html {
    let member_ctx = use_context::<TMemberContext>().unwrap();
    let chat_ctx = use_context::<TChatContext>().unwrap();

    let author = message.sender.id.eq(&member_ctx.member.id);
    let editing = use_state(|| false);
    let content = use_state(|| message.content.clone());
...

    {
        ...
        use_effect_with(chat_ctx.thread.clone(), move |new_thread| {
            if let Some(updated_message) = new_thread.thread.iter().find(|m| m.id == message.id) {
                content.set(updated_message.content.clone());
            }
            || ()
        });
    }
    ...
    let on_save_click = {
        ...
        Callback::from(move |event: MouseEvent| {
            ...
            spawn_local(async move {
                ...
                if let Ok(message) = api_put::<Message>(format!("/message/{}", message.id), form_data).await {
                    if let Ok(thread) = get_message_thread(chat_ctx.current_chat.id).await {
                        chat_ctx.dispatch(ChatDispatch::UpdateCurrentThread(thread));
                        editing.set(false);
                    }
                }
            });
